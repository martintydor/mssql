...
--
--Description:
--Script generates SQL script for permissions setup as its result. Then it is possible just to copz commands from result window and use them in other Db 
--
--!!!!!!!!!!!!!! It works on Azure !!!!!!!!!!!
--
...
SELECT  'IF NOT EXISTS (SELECT [name] FROM sys.database_principals WHERE [name] = ' + SPACE(1) + '''' + [name] + '''' + ') BEGIN CREATE USER ' + SPACE(1) + QUOTENAME([name]) + 
              case when [type] IN ('X','E') /*windows user / group */ then ' from EXTERNAL PROVIDER END; '
                     when [type] = 'S' then ' FOR LOGIN ' + QUOTENAME([name]) + ' WITH DEFAULT_SCHEMA = ' + QUOTENAME(isnull ([default_schema_name],'dbo')) + SPACE(1) + 'END; ' 
              end,
              4 AS [-- RESULT ORDER HOLDER --]
       FROM    sys.database_principals AS rm
       WHERE [type] IN ('X', 'S','E') -- windows users, sql users, windows groups
...
WHERE   USER_NAME(rm.member_principal_id) IN (  
      ...
      FROM sys.database_principals
              WHERE [principal_id] > 4 -- 0 to 4 are system users/schemas
                     and [type] IN ('X', 'S','E') -- S = SQL user, U = Windows user, G = Windows group, X – external group, E – external user
)
UNION
SELECT '' AS [-- SQL STATEMENTS --],
...
WHERE   [perm].[major_id] = 0
    ...
    AND [usr].[type] IN ('X', 'S','E') -- S = SQL user, U = Windows user, G = Windows group
UNION
...
print '----------------------------------------------------------------------------------------------------------------------'
print '--  This script is classified as TOP-SECRET and can not be used without bringing a bottle of wine to the author     --'
print '----------------------------------------------------------------------------------------------------------------------'
print ''
print 'DECLARE @Command nvarchar(max) = ''''
