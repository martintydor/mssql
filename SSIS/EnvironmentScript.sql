...
--
--Description:
--Script returns script for environment scripting
       ...
       DECLARE 
	   			@folder sysname = '<SSIS FOLDER NAME>', -- put SSIS folder name here
				@env sysname = '<ENV NAME>',  -- put SSIS evironemnt name here
				@project_id int,
        ...
        */        SELECT [env_var] = @tab + @tab + ', @' + ev.name + ' '  + 
					case when ev.base_data_type ='nvarchar' then 'nvarchar(4000)' else ev.base_data_type end	
					+ '= N''' + 
					replace (ISNULL(CONVERT(varchar(max), ev.value), '<REPLACE_ME>'),'ceclu0271.emea.zurich.corp',
					'CEAWD22776.zurich.com')
					+ ''''
                        , [name] = ev.name
        INTO #env_var
